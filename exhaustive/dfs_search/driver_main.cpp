...
struct Dispatcher {
    ...
    int getFreeWorker()
    {
        ...
        for (;;)
        {
            ...
            else return st.MPI_SOURCE;
        }
    }

    bool onLeaf(const int *verts, int tc, unsigned long long )
    {
        int s = slot++;
        if (pass2) { if (prefixMax[s] != L) return true; }
        else       { if (done[s])           return true; }
