// pfxio.hpp — read/write standalone prefix batch files (.pfx).
//
// A prefix is one canonical induced path of PREFIX_LENGTH vertices, always
// starting at vertex 0, plus its transitionCounter. That is the complete seed
// Search::dfs_from_prefix needs to resume searching — everything else is
// rebuilt from it. We therefore store, per prefix, only the PREFIX_LENGTH-1
// transition bytes (bit flipped at each step) and one transitionCounter byte.
//
// Layout (all header integers little-endian, so files move between machines):
//   header : 'P''F''X''1' | u16 N | u16 PREFIX_LENGTH | u64 record_count
//   record : (PREFIX_LENGTH-1) transition bytes | 1 transitionCounter byte
//
// record_count is written as 0 when a file is opened and patched to the true
// count when the file is closed, so an interrupted run still leaves every
// already-closed file valid.
#pragma once
#include "config.hpp"
...
static inline void pfxEncode(const int *verts, int tc, unsigned char *rec)
{
    for (int i = 1; i < PREFIX_LENGTH; i++)
        rec[i - 1] = (unsigned char)__builtin_ctz((unsigned)(verts[i] ^ verts[i - 1]));
