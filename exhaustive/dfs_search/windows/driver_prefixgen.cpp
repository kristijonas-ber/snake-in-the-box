// driver_prefixgen.cpp (Windows copy) — standalone canonical-prefix generator.
//
// Identical to the mac/Linux ../driver_prefixgen.cpp. It already carries the
// _WIN32 mkdir branch and uses only portable calls (fopen/fseek, signal, mkdir),
// so no changes are needed here; the only Windows-specific fix lives in pfxio.hpp
// (sib_ctz). Kept as a separate copy to match this folder's build layout.
//
// Streams every canonical prefix of PREFIX_LENGTH vertices to a rolling set of
// .pfx batch files in O(2^N + PREFIX_LENGTH) memory, no matter how many it emits.
// No MPI, so it builds and runs without MS-MPI.
//
// Usage:
